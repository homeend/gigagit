...
- **Branches `.` menu: *Show in Worktrees*.** On a branch checked out in a
  worktree (the current one included), the row switches to the Worktrees tab
...
- **`gg compare` takes `gg://` links on either side, and `gg link` can build
  ...
  link mixes freely with the old vocabulary (`gg compare gg://repo@a..b main`),
  because a link is a spelling, not a mode. `gg link` gained the flags that
  produce them: `--ref <branch|tag>` (the TIP, kept as a NAME so the link means
  "the branch", not whichever commit it sits on today; an unknown name is
  refused at the producer), `--pair <a>..<b>` (a CHANGE-SET, both halves
  resolved to full shas so the link travels), and `--bookmark <id>` /
  `--shelf <id>`, which attach a landing hint. `--cached`, `--rev`,
  `--preview`, `--ref` and `--pair` all name the target, so at most one may be
  given; `--bookmark` and `--shelf` are likewise mutually exclusive.

- **The `gg://` grammar gained `@ref:<name>`, `@<a>..<b>` and `?<hint>` — and
  `?` is no longer expressible inside a link.** A target may now be a branch or
  tag tip (`gg://repo@ref:main`, a POINT: the whole tree there) or git's two-dot
  change-set (`gg://repo@c1..c2`, BOUNDED: only what it changed, each half a
...
- **`gg compare` no longer refuses a "reversed" pair.** `gg compare @worktree
  ...
  `A` forward reads `D` reversed). That rule was git's own limitation showing
  through the CLI — git's diff only walks forward — not a statement about what
  a user may ask, so the comparison is asked forward and the answer inverted.
  The MCP `gg_compare_trees` tool gets the same totality: a reversed pair there
  used to answer `unsupported endpoint pair 1 → 3`, so the two compare
  frontends disagreed about what could be asked.

- **A frozen shelf entry can now be compared against the live index or working
  tree.** `gg compare shelf:<id> @staged` used to be refused with "a frozen
  shelf entry pairs only with a commit or another shelf entry". It answers the
  real question now — "is my shelved work already in my tree?" — scoped, as
  before, to the files the shelved commit changed. (Only a shelved commit whose
...
- **`gg compare`'s changed-file list is sorted by path.** It used to print
  ...
  could list `M z.txt` before `A a.txt` while the reverse direction of the same
  comparison came out sorted: one comparison, two orders. Every lane sorts now.
  The same applies to MCP's `gg_compare_trees`; the TUI's and browser's compare
  views still read git's order.

- **Fixed: a stale bookmark or version record could panic the whole TUI, and
  two Update-thread git reads could hang it.** Comparing a commit bookmark, and
  entering a Previous-versions row, fed a commit hash read straight back from a
  store — a bookmark's TOML record, or a version ref's trailer, which is split
  on whitespace without ever checking that a field is a sha — into a
  constructor that panics on anything else: one stale or hand-edited record
  took the process down. Both validate and decline with a status message now.
  Separately, resolving HEAD for the diff view and copying a
  commit's sha ran a synchronous git read on the Bubble Tea Update thread with
  no deadline — behind a queued write that is itself parked on a modal the
  frozen thread can no longer draw, that is a deadlock with no exit but killing
  gg. Both now run under a 300 ms deadline and decline (the sha copy falls back
  to the short hash it already had).
...
- **`model.Endpoint` is now always valid — the zero value is `EndpointInvalid` and every kind switch is exhaustive.** Endpoint's fields are unexported; the only way to construct one is through a validating constructor (`WorkTreeEndpoint`, `IndexEndpoint`, `CommitEndpoint`, `ShelfEndpoint`), so holding an Endpoint proves it is consistent. The zero value used to silently default to a valid working tree, which hid initialization errors; it is now `EndpointInvalid` and panics if you try to use it. Every switch on `EndpointKind` now has an explicit arm per kind plus a panicking default — previously `Display`, `FileRef` and `CacheTag` fell through to the commit case, so a kind they had not been taught about silently became a commit with an empty hash. A table test enumerates every kind, so a kind added without a row fails the build.

- **A commit-ish is resolved to a full sha before it becomes an Endpoint, so the diff cache is keyed on a sha that cannot move.** Endpoint.CacheTag() returns the sha verbatim, so it is the session diff-cache key; four call sites used to store a git rev-spec there instead — a branch name, `HEAD`, `HEAD~2`, or `<sha>^` — so a commit↔commit compare could serve a stale diff after the name moved. `branchTipHash` no longer falls back to returning the branch *name* when the name is unknown; it reports the miss and the compare is declined instead. `gg compare` with an unresolvable revision now exits 2 (a usage error) rather than letting it reach git. It cannot yet tell an unknown revision from a repository git failed to read, so a corrupt or locked repo also reports `unknown revision`.

- **A user-visible crash fixed, and every commit-ish resolver hardened.** Opening a WIP-vs-HEAD compare in a repo with no commits yet (`git init`, stage a file, select the Staged row, press enter) crashed the whole process; it now declines the compare with a notice. Separately — as hardening, not a fix to any shipped behaviour — Endpoint validation now requires a commit hash of 7..64 hex, which git's short-hash output cannot be relied on to satisfy: its width follows `core.abbrev`, legal down to 4. The compare resolvers were moved to full shas (`ResolveRev`) to match — `gg compare`, `gg web`'s branch↔branch compare and the MCP `gg_compare_trees` tool — while the TUI's branch compare declines the compare rather than opening one on a tip hash it cannot validate. A full sha also keys the session diff cache on something that cannot move.
...
- **TUI: cherry-pick applies every ◉ marked commit.** The Commits-panel
  ...
  and merge commits in the selection are refused with a clean message naming
  the offending commit. The engine op is now `CherryPick{Commits: []string}`;
  single-commit callers (CLI, MCP, shelf/bookmark lanes) behave exactly as
  before.
...
- **Repo switcher warns about slow filesystem mounts.** Rows whose repository
  ...
  selecting one shows "⚠ this repository is mounted on a foreign filesystem —
  switching may be very slow" as a tooltip strip beneath the row (an overlay,
  so the popup's height never changes as the cursor moves), and enter asks a
  Yes/No confirmation (default No; honors `[ui] disable_slow_op_confirm`)
  before committing to the switch — a whole-tree `git status` on such a mount
  can take a minute. Probes run async off-thread with a deadline, so a dead
  network mount can cost a row its marker but never freeze the picker.
