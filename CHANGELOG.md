...
- **Two user-visible crashes fixed.** Opening a WIP-vs-HEAD compare in a repo with no commits yet (`git init`, stage a file, select the Staged row, press enter) crashed the whole process; it now declines the compare with a notice. A repository with `core.abbrev` set below 7 (git's minimum is 4) made compares fail with a validation error and crashed the TUI outright on the branch-compare path, because git's short-hash output was rejected by Endpoint validation; resolution now uses full shas.
