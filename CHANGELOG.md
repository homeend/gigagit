...
- **A user-visible crash fixed, and every commit-ish resolver hardened.** Opening a WIP-vs-HEAD compare in a repo with no commits yet (`git init`, stage a file, select the Staged row, press enter) crashed the whole process; it now declines the compare with a notice. Separately — as hardening, not a fix to any shipped behaviour — Endpoint validation now requires a commit hash of 7..64 hex, so no code path can build an endpoint out of git's short-hash output, whose width follows `core.abbrev` (legal down to 4). Every site that builds one resolves to a FULL sha instead (`ResolveRev`): the TUI's branch compare, `gg compare`, `gg web`'s branch↔branch compare and the MCP `gg_compare_trees` tool. That also keys the session diff cache on a sha that cannot move.
