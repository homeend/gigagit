...
| `g` | **bookmark quick-switcher** — a centered, type-to-filter list of bookmarks (richly-addressed references to files anywhere: a worktree's working/index file, a commit/branch file, a shelf entry). `enter` diffs the bookmark against the current working-tree file, `m`+`m` compares two bookmarks, `p` pastes its contents to a path you type, `x` removes (confirms), `esc` closes. A bookmark to a working file is *live* (reflects later edits); to freeze bytes, use the shelf |
