...
| `w` | create a worktree **for the selected branch** (popup); `W` opens the same popup with **create & switch** as enter's default — one flow onto the branch's own clean directory (`<repo>.worktrees/<branch>`). A worktree on a **new** templated branch lives in the Branches `.` menu (**Worktree on a new branch from `<branch>`…**). Inside the popup: `enter` runs the popup's default, `w` create only, `W` create **and** switch; in new-branch mode additionally `e` edit the name and `p` pick a saved **branch prefix** (fills any `<user:…>` labels, then edit) |
