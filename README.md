...
| `R` | switch repository (popup: type to filter, `enter` to switch, `ctrl+d` to forget, `T` maximizes the popup to a near-fullscreen box) |
...
| `r` / `q` | reload / quit |
| `?` | help: searchable list of all key bindings (`/` to search; `↑`/`↓` or `j`/`k`, `ctrl+↑`/`ctrl+↓`, `pgup`/`pgdn`, mouse wheel to scroll; `T` maximizes the window to a near-fullscreen box; `q` closes) |
...
exotic keys). `/` filters as you type; `T` maximizes the popup to a near-fullscreen box; `esc` closes.
