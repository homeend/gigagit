...
`gg web`, right-click
a diff line, a file row or a commit row — inside an open merge preview those
rows copy the preview forms (`…@<target>...<source>[:<line>]`, new side only;
a deletion row copies the file), and a Previews row copies the pair's own
link. Quote a link that carries `#<hunk>` — an unquoted `#` starts a shell
comment.
