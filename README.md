...
| `f` | on the **Branches** panel: jump the cursor to the **checked-out branch** (the list scrolls to it; honors the current sort, and tells you when a `/` filter hides it or HEAD is detached); on the **Worktrees** panel: jump to the worktree gg is running in; on the **Remotes** panel: fetch all remotes; inside a diff: toggle full file ↔ changed lines (the browser UI has the same `f` and a **changes only** toolbar button; its folded runs read `⋯ N unchanged lines` and unfold on click, and the choice is remembered per machine) |
