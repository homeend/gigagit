...
| `F` | **files in the working tree** — a window over the whole left column listing every file on disk (tracked files minus deleted ones, plus untracked ones, marked `(untracked)`; no extra git scan — the untracked list is the Status panel's). `/` then a fuzzy query (`fvgo` → `files_view.go`) filters (`enter` keeps it, `esc` clears it); `↑↓/jk/pgup/pgdn` move and the right column **previews** the file on disk (it follows the cursor once it rests, and reloads when the file changes); `→` scrolls the preview, `←` returns. `enter` or `.` opens the file's actions: **View file content** (the full-screen viewer — an open file, see *Open files*), **Diff (HEAD ↔ working tree)**, **History**, **Blame**, **Edit in editor**, **Copy file name / path / absolute path**, **Copy file link**, **Commits touching this** (the git rows are for tracked files only). `ctrl+t` spreads the list over the whole screen; `esc` closes, back to where you were (over a diff, history or blame, back to it) |
