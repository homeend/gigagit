...
| `enter` | on the Worktrees panel: switch into the selected worktree; on the Status panel: full-screen side-by-side diff of the file (HEAD → working tree); on the files-view tree: diff of the file in the viewed commit. Inside the diff: `↑`/`↓` scroll, `pgup`/`pgdn` page, `ctrl+↑`/`ctrl+↓` jump between changes, `esc` closes |
