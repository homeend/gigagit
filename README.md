...
| mouse | click focuses the window under the cursor and selects the clicked row; **double-click acts as enter** on that row (drill into a commit, jump to a branch tip, run the selected action-menu or list-popup row); **right-click selects the row and opens its `.` action menu** (panels, files view, and the diff/history/blame views — a shortcut for `.`, never the only path); **clicking a tab name in a left-column header switches to that tab** (the same slots `ctrl+←/→` cycles); the wheel scrolls the hovered list (`[ui] wheel_step` rows per tick); text-entry popups and decision modals deliberately ignore clicks |
