...
1. **The lex is synchronous.** §4.2 says "in the loader's goroutine where one exists, else synchronously". No loader goroutine holds a `hunkpick.Doc`: `loadConflictFileCmd` returns marker text and `loadStageHunksCmd`/`loadUnstageHunksCmd` return two raw sides; the `Doc` is only assembled in `Update` (`model.go:3068/3090/3108`). So the "else" branch applies. The two sides are lexed concurrently and each is capped at `domain.MaxSyntaxBytes`. **Revised in the fix wave:** measurement (1.8 s on a 1 MB Go file, 390 ms on 200 KB) made a synchronous lex an unacceptable UI freeze, so the open sites now push the picker unlexed and return `hunkPicker.lexCmd`, whose `pickerLexedMsg` is applied only while that picker is still live; `withSyntax` remains as the synchronous form for tests.
