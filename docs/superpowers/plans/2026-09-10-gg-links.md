...
func noteAdd(svc *domain.Service, link *domain.Resolved, args []string, stdout, stderr io.Writer) int {
	...
	stored, err := svc.NoteAdd(ctx, model.Note{ /* … unchanged … */ })
	// … unchanged tail …
}
```
...
```go
	...
	// … the existing --type filter / JSON / render tail, unchanged …
```
...
$("diff-body").addEventListener("contextmenu", (e) => {
  ...
  // … the rest of the handler unchanged …
```
...
func TestExpandArgs(t *testing.T) {
	t.Parallel()
	got := ExpandArgs([]string{"diff", "gg://{{cwd}}/a.txt", "plain"}, `C:\src\repo`)
	want := []string{"diff", "gg://C:/src/repo/a.txt", "plain"}
...
4. **Ambiguity is refused only for WORKING-TREE and STAGED links; a COMMIT
   ...
   lists both. The cwd candidate always wins outright — the user ran the
   command there.
