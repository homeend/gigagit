...
(unfolding and loading a file to reach one), and a link or a `gg session
navigate` with a line lands on that LINE rather than the file header —
**TUI only:** `ctrl+↑`/`↓` step change to change inside one stacked file) · **pull
