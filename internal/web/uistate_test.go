...
func TestUIStateRoundTrips(t *testing.T) {
	ts := serve(t, uiServer(t))
	body := `{"sections":["tags","reflog"],"sidebar_hidden":true,"sidebar_width":310,"files_width":420,"graph":"off","diff_view":"changed","files_hidden":true,"sym_compare":true,"stacked_diff":true,"text_mode":"scroll"}`
	var put uiStateWire
	...
	st := getUIState(t, ts)
	if !st.Saved || st.SidebarWidth != 310 || st.FilesWidth != 420 || !st.SidebarHidden || st.Graph != "off" || st.DiffView != "changed" || !st.FilesHidden || !st.SymCompare || !st.StackedDiff || st.TextMode != "scroll" {
...
func TestUIStateSanitizesInput(t *testing.T) {
	ts := serve(t, uiServer(t))
	body := `{"sections":["reflog","bogus","tags","tags"],"sidebar_width":-40,"files_width":99999,"graph":"; rm -rf /","diff_view":"partial","text_mode":"diagonal"}`
	var put uiStateWire
	if code := putJSON(t, ts, "/api/uistate", body, "", &put); code != http.StatusOK {
		t.Fatalf("PUT code = %d", code)
	}
	...
	if put.TextMode != "wrap" {
		t.Fatalf("text_mode = %q, want the wrap fallback", put.TextMode)
	}
}
