...
func TestUIStateRoundTrips(t *testing.T) {
	ts := serve(t, uiServer(t))
	body := `{"sections":["tags","reflog"],"sidebar_hidden":true,"sidebar_width":310,"files_width":420,"graph":"off","diff_view":"changed","files_hidden":true}`
	var put uiStateWire
	...
	st := getUIState(t, ts)
	if !st.Saved || st.SidebarWidth != 310 || st.FilesWidth != 420 || !st.SidebarHidden || st.Graph != "off" || st.DiffView != "changed" || !st.FilesHidden {
