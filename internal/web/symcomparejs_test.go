...
func TestSymmetricCompareIsWiredEverywhere(t *testing.T) {
	...
	for _, c := range []struct{ file, want, why string }{
		...
		{"linkcompare.js", `const busy = "comparing… reading both sides";`, "every entry says it is working, not only the dialog"},
		{"linkcompare.js", `opLine(busy + " (" + Math.round(`, "a compare slower than opLine's 30s expiry keeps its line (re-posted with the time waited)"},
		...
		{"index.html", "<div id=\"diff-top\">\n    <div id=\"sym-dir\"", "the direction bar sits in the sticky wrapper, or change › scrolls it away"},
		{"style.css", `#diff-top { position: sticky; top: 0;`, "the wrapper is what stays pinned"},
		{"style.css", `#diff-top #diff-header { position: static; }`, "two sticky siblings at top 0 paint over each other"},
