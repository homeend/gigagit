...
func TestInViewSearchStaticWiring(t *testing.T) {
	...
	checks := []struct{ file, want, why string }{
		...
		{"style.css", `table.diff .hit, #blame-body .hit, #help-body .hit {`, "hit styling must be scoped to every host, like the .tk-* classes"},
		{"style.css", `table.diff .hit.cur, #blame-body .hit.cur, #help-body .hit.cur {`, "the current hit paints differently in every host"},
		...
		// The ? help overlay is the third host: the same bar over its rows,
		// painted by rewrapping text nodes (the rows carry <b> and <code>).
		{"index.html", `id="help-search"`, "the help overlay's search bar"},
		{"index.html", `id="help-search-input"`, "the help bar's query field"},
		{"index.html", `id="help-body"`, "the help rows scroll in a body of their own, under the bar"},
		{"style.css", `#help-search.hidden { display: none; }`, "per-ID hidden rule — the bar would never hide"},
		{"style.css", `#help-body { overflow-y: auto;`, "the body, not the box, is the scroll container"},
		{"menus.js", `$("help-body")`, "registered help rows land in the body, where the search walks"},
		{"helpsearch.js", `function helpSearchKey(`, "the help host's key hook"},
		{"helpsearch.js", `bindSearchBar("help-search"`, "the help host mounts the shared bar"},
		{"helpsearch.js", `from "./inviewsearch.js"`, "the help host uses the shared engine"},
		{"helpsearch.js", `createTreeWalker(`, "hits are painted by rewrapping the row's text nodes, so <b> and <code> survive"},
		{"ops.js", `e.target === $("help-search-input")`, "keys typed into the help bar must not be swallowed (help preventDefaults every key)"},
		{"ops.js", `helpSearchKey(e)`, "the help layer routes / @ ] [ esc to the search first"},
		{"app.js", `"./helpsearch.js"`, "the host module is loaded"},
