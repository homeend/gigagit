...
func cmdAdd(svc *domain.Service, dir string, args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	res, err := runOperation(context.Background(), svc,
		engine.Stage{Paths: repoPathspecs(svc, dir, fs.Args()), All: *all}, dec, stderr)
	return finish(res, err, stdout, stderr)
}

// cmdUnstage implements `gg unstage <path>...` — remove paths from the
// index, keeping working-tree content.
func cmdUnstage(svc *domain.Service, dir string, args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "usage: gg unstage <path>...")
		return 2
	}
	res, err := runOperation(context.Background(), svc,
		engine.Stage{Paths: repoPathspecs(svc, dir, args), Unstage: true}, cliDecider{}, stderr)
	return finish(res, err, stdout, stderr)
}
