...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case dataLoadedMsg:
		...
		if msg.err == nil {
			...
			m.cfg = msg.cfg
			// Honor [debug] log_operations on first load. Only ever auto-ENABLES
			// (never disables): the , menu toggle persists its choice to config, so
			// a later reload that sees the same value is a no-op either way.
			if m.opLog != nil && m.cfg.Debug.LogOperations && !m.opLog.on {
