...
func helpContent() []contentLine {
	...
	return []contentLine{
		...
		r("ctrl+\\", i18n.T("the agent-sessions popup, from anywhere (even inside a console): every session this gg runs, grouped repo → worktree; enter opens one, k kills (asks once), x removes an exited one, / filters. Quitting gg with sessions alive opens it in quit mode ([Q] kill all and quit)")),
		r("ctrl+]", i18n.T("step out of a focused agent console (a maximised one shrinks back to the Commits column); every other key goes to the agent while its console is focused")),
