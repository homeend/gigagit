...
func helpContent() []contentLine {
	...
	return []contentLine{
		...
		r("F", i18n.T("files in the working tree: a window over the left column listing every file on disk (untracked marked); / then a fuzzy query (fvgo → files_view.go) filters, ↑↓ move and the right column previews the file, enter or . opens its actions (View file content, Diff, History, Blame, Edit in editor, Copy name/path/absolute path/file link), → scrolls the preview, ctrl+t full screen, esc closes")),
		...
		r("#", i18n.T("show a commit or open a gg:// link: type a commit-ish (SHA / ref) or paste a gg:// link; enter resolves it — a commit opens its files in the files-view, a link lands on its place (a link into another checkout asks before switching there)")),
		r("# ↓", i18n.T("in the # prompt, ↓ picks from the gg:// links you copied — newest first, from the TUI, the CLI and the web alike")),
		r("", i18n.T("Compare with link… (ctrl+p palette): compares any two gg:// links — type or paste them, or press ↓ to pick from the links you copied; ctrl+s swaps the sides. A link that names a whole branch, tag or commit gets a base row under it: enter on that row bounds the link (a branch against its upstream or trunk, a commit against its parent) so the comparison lists only what it changed; leave the row alone to compare the whole tree")),
		...
		r("", i18n.T("symmetric merge preview: m marks branch A, m on branch B → \"Symmetric merge preview…\", then type the base — saved as one Previews row (sym) comparing what A and B would each bring into the base; enter opens it, . opens either side's merge preview")),
		...
		r("d", i18n.T("remove the selected preview after a confirm")),
		r("s", i18n.T("save the reversed pair (target → source) as a second preview")),
		h(i18n.T("Pull requests panel")),
		r("", i18n.T("a fifth tab in the top-left box, present only when a forge CLI (GitHub's gh, signed in) can read this repository's pull requests; read-only — gg never posts to the forge. Open pull requests are listed; one gg already knows stays listed, dimmed and marked, after it is merged or closed")),
		...
		r("A", i18n.T("search the forge for pull requests of any state — closed and merged ones included: the text goes to the forge's own search (GitHub qualifiers such as author:name or head:branch work), [tab] cycles all / closed / merged / open, a bare number opens that pull request; the 50 newest-updated matches are listed and act like rows of the tab (enter, i, y); closing a diff or the details returns to the results, and the last search is remembered for the session")),
		...
		r("s", i18n.T("skip the region: resolve it with no lines from either side and move to the next undecided one (s again un-skips); in the stage/unstage pickers, reset the hunk to its default and step on")),
		...
		r("", i18n.T("Save to previews / Save reversed to previews (.-menu, exactly 2 commits marked with m): stores the diff between the two commits as a named entry in the Previews tab — the commits are frozen, so the entry never follows a branch; older → newer, the direction Compare selection diffs in")),
		...
		r(".", i18n.T("tree side: View file — show the file's content at this commit (no diff) in the right pane ([↑/↓] scroll, [alt+↑/↓] move the line cursor, [space] starts a line selection and [enter] copies it, [ctrl+w] view mode, / or @ find text forward / backward and ] [ step the hits, [esc] cancels a search being typed, then clears the line selection, then a kept query, then closes, [←] back to the tree; the preview's own . menu carries Copy line and Copy selected lines); Open in external editor — open that content in $VISUAL/$EDITOR, read-only")),
		...
		r("S", i18n.T("stacked view: every file of the list in one scroll, a header per file with its diff below (the choice is remembered; S again returns to the single file under the cursor)")),
		r("n/p (stacked)", i18n.T("next / previous change across the WHOLE stack — the files are one scroll, so the walk crosses from one into the next, unfolding and loading a file to reach a change in it; home/end go to the top / bottom of the stack and never step to another file")),
		r("N/P (stacked)", i18n.T("next / previous FILE of the stack (its header); J jumps to one by name")),
		...
		r("ctrl+↑/↓ (stacked)", i18n.T("stacked: previous / next change INSIDE the file under the cursor — it stops at that file's ends, where n/p would carry on into the next file")),
		...
		r("H (stacked)", i18n.T("stacked: stage (or, in the Staged section, unstage) hunks of the file under the cursor — the region picker opens over the stack and returns to it, and the stack re-reads the file the staging changed")),
		r("", i18n.T("the changes-only fold (f) is still single-file: press S to go back to one file for it")),
		r("/ @ ] [ (stacked)", i18n.T("stacked: / and @ search every file whose diff has arrived; ] and [ step hit to hit across the whole stack, unfolding and loading a file to reach a hit, and a + on the counter says files are still unsearched")),
		...
		r("H", i18n.T("working-tree diff: open the region/line staging picker for this file (the Staged section unstages instead); the diff re-reads itself when the round changes it, and closes when nothing is left to show here")),
		...
		r("esc", i18n.T("backs out one thing at a time: a search being typed, then the line selection, then a kept query, then the view")),
		...
		r("/ @", i18n.T("find text forward / backward (enter keeps it, esc cancels)")),
		r("] [", i18n.T("next / previous hit (wraps around)")),
		r("d", i18n.T("highlight lines by commit age — -7d younger than, +30d older than, +1d -7d between (a bare span means younger than; +w = one week); uncommitted lines are age 0; press again to change it — the highlight is off whenever blame opens")),
		r("D", i18n.T("turn the age highlight off")),
		...
		r("colours", i18n.T("Theme colours… edits any colour role of the active theme: / filters, enter edits the value in place (the whole screen previews it live), d restores the built-in default, D resets the whole theme after a confirm (drops the global [themes.<name>] table; repo values stay), t cycles the theme, ctrl+t fullscreen. Saving writes one line to the global config's [themes.<name>]; roles pinned by the repo .gg.toml show (repo) and stay read-only")),
