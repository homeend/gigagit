...
func helpContent() []contentLine {
	...
	return []contentLine{
		...
		r("z", i18n.T("cycle text display: cutoff / wrap / scroll")),
		...
		r(".", i18n.T("tree side: View file — show the file's content at this commit (no diff) in the right pane ([↑/↓] scroll, [z] view mode, [esc] close, [←] back to the tree); Open in external editor — open that content in $VISUAL/$EDITOR, read-only")),
		...
		r("enter", i18n.T("drill into the stash's file tree with focus on the tree (like enter on a commit); with the tree already open, enter on the list opens the stash actions")),
		r(".", i18n.T("stash actions menu: apply / pop / drop (drop confirms) and Copy stash ref")),
		...
		r("(key)", i18n.T("run the action with that key; lists context actions only (row + panel)")),
		r("↑/k ↓/j", i18n.T("move; enter runs the highlighted action")),
		r("/", i18n.T("filter the list by name (enter keeps, esc cancels)")),
		r("z", i18n.T("cycle text display: cutoff / wrap / scroll")),
		r("esc", i18n.T("close")),
