...
// enter in the history view promotes the selected commit's diff to the
// full-screen surface (m.diffView) and clears the layer stack so the diff owns
// key routing (a layer would otherwise intercept keys above it).
func TestHistoryEnterOpensFullScreenDiff(t *testing.T) {
	...
	if m.diffView == nil {
		t.Fatal("enter should open the full-screen diff (m.diffView set)")
	}
	if m.topLayer() != nil {
		t.Fatal("the history layer must be cleared so the diff owns key routing")
	}
	if m.diffTag == "" {
...
// The full-screen loader must yield a diffMsg (lands in m.diffView), not the
