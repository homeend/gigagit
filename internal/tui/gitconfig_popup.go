...
func (p *gitConfigPopup) box(m Model) string {
	...
	default:
		...
		// Wrap mode hang-indents continuations at the row text (intrinsic to
		// wrap mode; keys start at column 0, so it is a no-op for most rows).
		// configRowDecorator only ever decorates a row's first visual line,
		// so the indent cannot drift its column spans.
...
func (p *gitConfigPopup) editBox(inner, textW int) string {
	...
	if e.useField {
		...
		for i, opt := range e.options {
			...
			row := "  " + label
			if i == e.optSel {
				row = st().selectedRow.Render("> " + label)
			}
...
// configRowDecorator (see its doc comment). Baking st().dim.Render in here
...
// the dim-style wrap handling in commit_ident.go: a wrapped continuation
...
func configRowDecorator(row model.GitConfigRow, keyW, localW, globalW int) rowDecorator {
	...
	if !row.LocalSet {
		spans = append(spans, coloredSpan{Start: localStart, Length: localW, Style: st().dim})
	}
	if !row.GlobalSet {
		spans = append(spans, coloredSpan{Start: globalStart, Length: globalW, Style: st().dim})
	}
	if len(spans) == 0 {
		return nil
	}
