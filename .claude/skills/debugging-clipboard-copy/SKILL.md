...
| WSL, interop off, `wl-copy` installed, `WAYLAND_DISPLAY` SET, still nothing (`wl-copy` by hand: "Failed to connect to a Wayland server") | **The variable names a socket that is not there**: `/run/user/<uid>` missing, the live socket is `/mnt/wslg/runtime-dir/wayland-0`. | Already handled (2026-09-20): `resolveWaylandDisplayIn` trusts only LIVE sockets and falls back to WSLg's dir; `nativeCopyCmd` hands the resolved display to the child whenever it differs from the inherited one. Check with `ls $XDG_RUNTIME_DIR/wayland-* /mnt/wslg/runtime-dir/`. |
