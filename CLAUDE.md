...
| `linknav`    | The one `gg://` link → navigate builder (`Resolve`, `Command`, `HunkLine`, `AtLink`, `RepoOnly`) shared by the CLI (`gg open`, `gg session navigate`) and the TUI's `#` paste prompt; domain + steer + model, never a frontend. |
