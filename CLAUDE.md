...
| `gitwatch`   | Pure fsnotify wrapper + `.git`-layout path→source map (`Plan`) + drvfs `Supported` gate + debounced `Watcher`; no git/TUI/domain imports. Backs event-driven auto-refresh (Phase D). TUI maps `gitwatch.Source`→`sourceKey` and feeds `enqueueDue`. Branches/remotes watch support is Phase D2. |
...
| `config`     | TOML config (`.gg.toml`), field-level overlay (defaults→global→repo), `<seq>` counters. Sections: `[worktree]`/`[ui]`/`[debug]`/`[refresh]`. The `[refresh]` section holds the master `enabled` bool, per-source interval seconds (`status`/`branches`/`remotes`/`worktrees`/`tags`/`reflog`/`feed`/`fetch`, all defaulting to 0/off), `min_seconds` (default 10; floor on any auto-refresh interval so cheap sources don't poll faster), and per-source watch bools (`worktrees_watch`/`reflog_watch`, default `false` — Phase D1; branches/remotes Phase D2). No adaptive keys (`disable_adaptive`/`max_read_seconds`/`backoff_factor` are gone). Read-only at runtime **except** four non-destructive line-edit writers: `SetGlobalDebugLogOperations` (`[debug] log_operations`), `SetGlobalRefreshEnabled` (`[refresh] enabled`), `SetRefreshInterval` (`[refresh] <source>` in the **repo** `.gg.toml` — backing the Settings "Refresh rates" inline editor), and `SetRefreshWatch` (`[refresh] <source>_watch` in the **repo** `.gg.toml` — backing the `w` toggle in the same editor). Both `gg config init` and `gg config populate` are generated from the `settingDocs` registry in `template.go` (single source of truth); `populate` is an additive, comments-only top-up that appends missing keys as `[populated]`-marked lines and never overwrites user values. |
