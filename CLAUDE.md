...
| `config`     | TOML config (`.gg.toml`), field-level overlay (defaults→global→repo), `<seq>` counters. Sections: `[worktree]`/`[ui]`/`[debug]`/`[refresh]`. The `[refresh]` section holds the master `enabled` bool, per-source interval seconds (`status`/`branches`/`remotes`/`worktrees`/`tags`/`reflog`/`feed`/`fetch`, all defaulting to 0/off), and Phase C adaptive keys: `disable_adaptive` (default `false` ⇒ adaptation on), `max_read_seconds` (default 10; cutoff above which a source drops to manual-only), `backoff_factor` (default 10; multiplies avg read to derive effective interval). Read-only at runtime **except** three non-destructive line-edit writers: `SetGlobalDebugLogOperations` (`[debug] log_operations`), `SetGlobalRefreshEnabled` (`[refresh] enabled`), and `SetGlobalRefreshDisableAdaptive` (`[refresh] disable_adaptive`, the third writer — backing the Settings "Adaptive intervals" toggle). |
