...
| `config`     | TOML config (`.gg.toml`), field-level overlay (defaults→global→repo), `<seq>` counters. Sections: `[worktree]`/`[ui]`/`[debug]`. Read-only at runtime **except** `SetGlobalDebugLogOperations` (a non-destructive line edit of the global file's `[debug] log_operations`, backing the TUI operation-log toggle). |
...
| `observ`     | Observability: span ring buffer, tracing, redaction, panic dump. `SetSpanSink(w)` mirrors every recorded span (each op + each git invocation, redacted JSON lines) to a writer — the basis of the TUI **operation log** (`internal/tui/oplog.go`, toggled from `,` Settings) and the `--time-track` CLI flag. |
