...
| `preflight`  | Pure feature-requirement registry + resolver: features declare data-format ranges, a minimum git version, a usable forge CLI and a criticality (a `Silent` feature never raises a notice); `Resolve` maps probe results to Satisfied/Repairable/Unsatisfiable. DAG leaf; `domain` runs the probes and owns the migration op. |
...
| `cli`        | Scriptable frontend; `cliDecider` answers forks from flags or stdin. Agent-facing terse verbs (`log`/`diff`/`show`/`add`/…), `gg batch`, `gg review`, `gg apply`, `gg versions`, `gg unlock`, `gg compare`, `gg note`, `gg links`, `gg pr` (read-only), `gg skill path`. |
