...
| `config`     | TOML config (`.gg.toml`), field-level overlay (defaults→global→repo), `<seq>` counters. Sections: `[worktree]`/`[ui]`/`[debug]`/`[refresh]`. The `[refresh]` section holds the master `enabled` bool, per-source interval seconds (`status`/`branches`/`remotes`/`worktrees`/`tags`/`reflog`/`feed`/`fetch`/`remote_tags`, all defaulting to 0/off), `min_seconds` (default 10; floor on any auto-refresh interval so cheap sources don't poll faster), and `disable_remote_tags_auto` (inverted bool, default `false` = auto-refresh ON; independent of `enabled` — disabling it suppresses the `srcTags`-arrival auto-enqueue, not the timed `remote_tags` interval). No adaptive keys (`disable_adaptive`/`max_read_seconds`/`backoff_factor` are gone). Read-only at runtime **except** four non-destructive line-edit writers: `SetGlobalDebugLogOperations` (`[debug] log_operations`), `SetGlobalRefreshEnabled` (`[refresh] enabled`), `SetRefreshInterval` (`[refresh] <source>` in the **repo** `.gg.toml` — backing the Settings "Refresh rates" inline editor), and `SetGlobalDisableRemoteTagsAuto` (`[refresh] disable_remote_tags_auto` in the **global** config — backing the Settings "Auto remote-tag refresh" toggle). Both `gg config init` and `gg config populate` are generated from the `settingDocs` registry in `template.go` (single source of truth); `populate` is an additive, comments-only top-up that appends missing keys as `[populated]`-marked lines and never overwrites user values. |
