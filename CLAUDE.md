...
| `tui`        | Bubble Tea Elm-style UI (value-receiver `Model`, panels, layer stack, modal Decider, async ops). Per-source refresh registry + background auto-refresh lane + file-watch; command palette; popups embed `popupMax` for ctrl+t maximize; agent consoles (`console.go`) over the Commits column with Worktrees session sub-rows and a `tea.WithFilter` quit guard. New ops must be mapped in `opAffectedSources`. |
