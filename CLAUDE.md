...
|--------------|----------------|
| `engine`     | Operations + the `Event`/`Decider`/`Result` contract. Smart ops (`SmartPull`, `SmartSwitch`, `SmartMerge`, `SmartRebase`), `Commit`, `Push`, `Stash`, worktree/patch/export/config ops, AI capture ops (`GenerateMessage`, `ReviewChanges`, `CompleteConflict`, `ConflictAgent`) as `CaptureTask`s (`Prepare`/`Collect`; `Run` = prepare → capture → collect), branch-version snapshots, `ApplyMigration` (which runs the `MigrationAction` it is handed — `DiscardRefs` or a domain-supplied converter — and only then stamps the format marker). Ops act on a `GitOps` interface; seams: `HookRunner`, `CaptureRunner`. English event prose + localizable `Msg{Format,Args}` channel via `msg.go` helpers only. |
...
| `domain`     | Frontend-facing command + query layer: `Execute` under the repo-gate reservation; singleflight-coalesced reads (`Snapshot`/`Status`/`CommitFeed`/…); the cached `Differ`; conflict/paused-op detection; review + conflict-complete report wrappers; branch-version and repo-health queries; once-per-session forge detection + read-only pull-request queries (`PullRequests`/`PRComments`/`PRFetchOp`/`PRPair`); the process-global AI-task scheduler `Tasks()` (per-key FIFO, `[tasks] max_parallel` cap, headless via `Execute`, interactive via a session + `$GG_MESSAGE_FILE` watcher). Owns the **compare algebra**: `EvalEndpoint`/`EvalLink` turn an endpoint or a `gg://` link into a `FileSet` (bounded = a key set, unbounded = a whole tree) and `CompareSets` is total over the 2×2 — no frontend screens a pair. |
...
| `tui`        | Bubble Tea Elm-style UI (value-receiver `Model`, panels, layer stack, modal Decider, async ops). Per-source refresh registry + background auto-refresh lane + file-watch; command palette; popups embed `popupMax` for ctrl+t maximize; agent consoles (`console.go`) over the Commits column with Worktrees session sub-rows and a `tea.WithFilter` quit guard; AI-task launch dialog, Headless tab and ◆ rows over `domain.Tasks()`. New ops must be mapped in `opAffectedSources`. |
