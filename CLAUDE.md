...
| `cli`        | Scriptable frontend; `cliDecider` answers forks from flags or stdin. Agent-facing terse verbs (`log`/`diff`/`show`/`add`/…), `gg batch`, `gg review`, `gg apply`, `gg versions`, `gg unlock`, `gg compare` (incl. `--save`/`--saved`/`--list`), `gg note`, `gg links`, `gg pr` (read-only), `gg skill path`. |
