...
| `markdown`   | Pure parser for the GitHub markdown subset forge text is written in: `Parse` → a bounded JSON block tree (the web's wire shape) with chroma token runs on fenced code, `Summary` (the review-thread summary/rationale split), http(s)-only link destinations. DAG leaf: stdlib + `syntax`. Owned by `domain`; the web paints the tree (`static/markdown.js`), never parses. |
